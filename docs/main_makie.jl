...
function add_basemap(ax)
  # plot!(ax, shp, strokecolor=:black, color=nan_color, strokewidth=0.5)
  plot!(ax, shp, color=:black, linewidth=0.5)
  hlines!(ax, [0], color=:grey, linewidth=1.5, linestyle=:dash)
  rm_ticks!(ax)
end

function GridLayoutBase.GridPosition(ax::Axis)
